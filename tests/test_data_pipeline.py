...
def test_collector_anonymizes_personal_data(tmp_path):
    ...
    record = collector.anonymize({
        "student_id": "123", "full_name": "Ama Mensah", "score": 92,
    })
    assert record["student_id"] != "123"
...
def test_preprocessor_deduplicates(tmp_path):
    raw = tmp_path / "raw"
    raw.mkdir()
    value = {"question": "What is WAEC?", "answer": "A regional examination body for West Africa."}
...
def test_splitter_is_deterministic_and_complete(tmp_path):
    ...
    rows = [
        {"instruction": f"Question {i}", "input": "", "output": f"Answer {i}", "category": "education"}
        for i in range(20)
    ]
